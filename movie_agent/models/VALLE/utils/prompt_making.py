...
text_tokenizer = PhonemeBpeTokenizer(tokenizer_path="./models/VALLE/utils/g2p/bpe_69.json")
...
def make_transcript(name, wav, sr, transcript=None):
    ...
    if transcript is None or transcript == "":
        ...
        torchaudio.save(f"./Results/prompts/{name}.wav", wav, sr)
        lang, text = transcribe_one(whisper_model, f"./Results/prompts/{name}.wav")
        lang_token = lang2token[lang]
        text = lang_token + text + lang_token
        os.remove(f"./Results/prompts/{name}.wav")
        whisper_model.cpu()
    else:
        text = transcript
